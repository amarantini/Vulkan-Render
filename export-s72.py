...
def write_mesh(obj):
	...
	if len(mesh.color_attributes) == 0:
		...
		colors = mesh.color_attributes.active_color.data
		if len(mesh.color_attributes) != 1:
			print(f"WARNING: multiple vertex color layers on '{mesh.name}'; using the active one ('{mesh.color_attributes.active_color_name}') in export.")
	...
	for tri in mesh.loop_triangles:
		for i in range(0,3):
			...
			if colors != None: 
				color = colors[loop.vertex_index].color
			else: 
				color = (1.0, 1.0, 1.0)
